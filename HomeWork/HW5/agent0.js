...
class Agent {
  constructor(pos, halfSize) {
    ...
    this.mesh = agentMesh (this.halfSize);
    this.MAXSPEED = 50;
    this.ARRIVAL_R = 30;
  ...
  update(dt) {
    ...
    var i=0, projMin = -1, minTmp = 10000;
    ...
    for (i = 0; i < obs.length; i++) {
      ...
      const REACH = 50;
      const K = 5;
      let distToTarget = this.target.pos.distanceTo(this.pos);
      if (proj > 0 && proj < REACH && proj < minTmp && distToTarget > proj ) {
        minTmp = proj;
        projMin = i;
      }
    }
